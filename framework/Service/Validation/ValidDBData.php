...
class ValidDBData {
    ...
    protected function getColumnInfo($strTableName) {
        ...
        //2.数据库获取
        $strSql = 'select col_name,cname,is_null,data_len,sql_type,data_type from interview_ecolumn where table_name=:table_name';
        $arrParams = [':table_name' => $strTableName];
        $arrColumnInfo = $this->objDB->setMainTable('interview_ecolumn')->select($strSql, $arrParams);
     ...
     * @param array $arrNeedCheck 需要检查的字段值，如array("invite_id"=>1,"jobseek_id"=>1)
     * @param array $arrCheckCol 需要检查的字段，如果没有设置，默认检查$arrNeedCheck中的所有字段，如array("invite_id")
     * @param bool $blnCheckStrEmpty 字符串类型的字段，如果数据库标记为非空，是否需要检查字段值是否为空，默认为true
     * @return array $arrCheckResult 检查的结果，如array(array("col_name"=>"invite_id","cname"=>"面试邀请id","err_msg"=>"类型错误","err_code"=>"02"))
     */
    public function check($strTableName, $arrNeedCheck = array(), $arrCheckCol = array(), $blnCheckStrEmpty = true) {
        $arrCheckResult = [];
        //没有需要检查数组
